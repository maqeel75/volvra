...
-- Nothing has been released, so there is exactly one schema version: 1.
-- Databases created during development may hold an older shape; the
-- compatibility block further down repairs those in place rather than
-- pretending each development step was a release.  Delete that block once
-- there are no pre-release databases left, and add real numbered migrations
-- from 2 onwards.
DO $adopt$
BEGIN
  IF to_regclass('volvra.change_log') IS NOT NULL
     AND NOT EXISTS (SELECT 1 FROM volvra.schema_version) THEN
    INSERT INTO volvra.schema_version (version, note)
    VALUES (1, 'adopted: pre-release install, schema repaired in place');
  END IF;
END
$adopt$;
...
-- v1 shape fixes, for a database that predates these columns.  They run against
-- the unpartitioned table, before the migration below reshapes it.
DO $patch_v1$
BEGIN
  -- Shape, not version: a pre-release database may lack these columns
  -- whatever its ledger says.
  IF to_regclass('volvra.change_log') IS NOT NULL
     AND NOT EXISTS (SELECT 1 FROM pg_attribute
                     WHERE attrelid = 'volvra.change_log'::regclass
                       AND attname = 'db_user' AND NOT attisdropped) THEN
...
-- ---------------------------------------------------------------------
-- Pre-release compatibility: partition an unpartitioned change_log
--
-- Guarded on the table's actual shape rather than on a version number,
-- because this repairs databases created during development rather than
-- migrating between releases.  Delete this block once no pre-release database
-- remains.
--
-- The history is the product.  This copies every row, keeps the ids, and only
-- drops the old table once the new one holds the same count.
-- ---------------------------------------------------------------------
DO $repair_partitioning$
DECLARE
  v_before bigint;
  v_after  bigint;
BEGIN
  IF (SELECT c.relkind FROM pg_class c
      WHERE c.oid = 'volvra.change_log'::regclass) = 'p' THEN
    RETURN;                      -- already partitioned
  END IF;

  RAISE WARNING 'volvra: repairing a pre-release schema -- partitioning change_log';

  SELECT count(*) INTO v_before FROM volvra.change_log;

  -- Keep the sequence alive across the table swap.
  ALTER SEQUENCE volvra.change_log_id_seq OWNED BY NONE;
  ALTER TABLE volvra.change_log RENAME TO change_log_v1;
  ...
  -- A partition per month the old data actually spans.
  PERFORM volvra._create_partition(m)
  FROM (SELECT DISTINCT date_trunc('month', ts)::date AS m
        FROM volvra.change_log_v1) months;

  INSERT INTO volvra.change_log
    (id, table_name, op, pk, old_row, new_row, actor, db_user, txid, ts)
  SELECT id, table_name, op, pk, old_row, new_row, actor, db_user, txid, ts
  FROM volvra.change_log_v1;

  SELECT count(*) INTO v_after FROM volvra.change_log;

  IF v_after <> v_before THEN
    RAISE EXCEPTION 'volvra: migration copied % of % rows -- refusing to drop the original',
      v_after, v_before;
  END IF;

  DROP TABLE volvra.change_log_v1;

  UPDATE volvra.schema_version
     SET note = format('%s; repaired in place, %s history row(s) preserved',
                       note, v_before)
   WHERE version = 1;

  RAISE WARNING 'volvra: repaired, % history row(s) preserved', v_before;
END
$repair_partitioning$;
...
-- Pre-release compatibility: redaction metadata, added idempotently.  A fresh
-- install already declares these columns; a development database may not.
ALTER TABLE volvra.change_log ADD COLUMN IF NOT EXISTS redacted_at timestamptz;
ALTER TABLE volvra.change_log ADD COLUMN IF NOT EXISTS redacted_by text;
